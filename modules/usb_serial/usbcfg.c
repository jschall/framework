 ...
 * Modified again for use in OMD/Framework by Jonathan Challinger
 */
...
static void string_substitute(const char *str, char *str2)
{
    ...
    if (string_contains(str, "%BOARD%")) {
        new_len += strlen("OMD_Framework") - 7;
    }
    ...
    while (*str) {
        ...
        if (c == '%') {
            if (strncmp(str, "%BOARD%", 7) == 0) {
                memcpy(p, "OMD_Framework", strlen("OMD_Framework"));
                str += 7;
                p += strlen("OMD_Framework");
                continue;
            }
...
{
    setup_usb_string(&vcom_strings[1], "OpenMotorDrive", vcom_buffers[0]);
    setup_usb_string(&vcom_strings[2], "%BOARD%", vcom_buffers[1]);
